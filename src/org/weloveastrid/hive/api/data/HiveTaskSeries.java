...
public class HiveTaskSeries extends HiveData {

    private final HiveTaskList list;
    ...
    private final HiveTask task;

    private final LinkedList<String> tags;

    private final HiveTaskNotes notes;
    ...
    public HiveTaskSeries(HiveTaskList list, String id, Date created, Date modified, String name,
            String source, HiveTask task) {
        this.list = list;
        this.id = id;
    ...
    public HiveTaskSeries(HiveTaskList list, Element elt) {
        this.list = list;
        ...
        if (children.size() > 1) {
            ...
            HiveTask selectedTask = new HiveTask(children.get(0));
            for(Element element : children) {
                HiveTask childTask = new HiveTask(element);
            ...
            task = new HiveTask(child(elt, "task"));
        }
        notes = new HiveTaskNotes(child(elt, "notes"));
